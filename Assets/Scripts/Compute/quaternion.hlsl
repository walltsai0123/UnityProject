...
float4x4 quaternion_to_4x4matrix(float4 quat)
{
...
float3x3 quaternion_to_3x3matrix(float4 quat)
{
...
float3x3 rotate_matrix(float4 q, float3x3 mat)
{
    return mul(quaternion_to_3x3matrix(q), mat);
}

float3x3 rotate_matrix(float3x3 mat, float4 q)
{
    return mul(mat, quaternion_to_3x3matrix(q));
}
