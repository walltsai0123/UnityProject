...
namespace XPBD
{
    public abstract class Joint : Constraint
    {
        ...
        protected REAL linearDamping = 0;

        [SerializeField, Min(0)]
        protected REAL angularDamping = 1; 

        public override void SolveVelocities(REAL dt)
        {
            REAL3 dv = (body2.vel - body1.vel) * math.min(linearDamping * dt, 1f);
            REAL3 domega = (body2.omega - body1.omega) * math.min(angularDamping * dt, 1f);

            // linear part
            REAL3 p = dv / (body1.InvMass + body2.InvMass);
            body1.vel += p * body1.InvMass;
            body2.vel -= p * body2.InvMass;

            // angular part
            REAL3 n = math.normalizesafe(domega, REAL3.zero);
            REAL w1 = math.mul(n, math.mul(body1.InertiaInv, n));
            REAL w2 = math.mul(n, math.mul(body2.InertiaInv, n));
