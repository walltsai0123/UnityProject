...
namespace XPBD
{
    public class SoftBodySystem : IDisposable
    {
        ...
        public void GetParticleBodyAndIndex(int index, out SoftBody softBody, out int localIndex)
        {
            softBody = null;
            localIndex = -1;

            if (index < 0 || index >= VerticesNum) return;

            int bodyStart, bodyEnd;
            for (int i = 0; i < verticesOffset.Count - 1; i++)
            {
                bodyStart = verticesOffset[i];
                bodyEnd = verticesOffset[i + 1];

                if (index >= bodyStart && index < bodyEnd)
                {
                    softBody = softBodies[i];
                    localIndex = index - bodyStart;
                    return;
                }
            }

            // Last body check
            bodyStart = verticesOffset[^1];
            bodyEnd = VerticesNum;
            if (index >= bodyStart && index < bodyEnd)
            {
                softBody = softBodies[^1];
                localIndex = index - bodyStart;
                return;
            }
        }
