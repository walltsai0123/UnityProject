    ...
    public class Wheel : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            void UpdateVisual(float speed, float steerAngle)
            {
                ...
                wheelVisualTransform.SetPositionAndRotation(pos, rot);
        ...
        public void UpdateWheelForce(float acceleration)
        {
            if(acceleration > 0)
                ...
                wheelCollider.brakeTorque = -acceleration;
            if (IsGrounded == false)
                return;

            float N = wheelHit.force;
            // Apply cancel force.
            ...
            var driveForce = GetTractionForce();
            vehicleRigidbody.AddForceAtPosition(driveForce, wheelCollider.transform.position, ForceMode.Impulse);
            
            lateralForce = lateralCancelForce;
            dForce = driveForce;
            ...
            Vector3 GetSkiddingCancelForce()
            {
                ...
                localV = wheelVelocity;
                return skiddingCancelForce;
            }
            Vector3 GetTractionForce()
            {
                var pointVelocity = wheelCollider.attachedRigidbody.GetPointVelocity(wheelHit.point);
                var wheelVelocity = Vector3.Dot(pointVelocity, wheelHit.forwardDir) * wheelHit.forwardDir;
                var wheelAngularVelocity = wheelCollider.rpm * 2 * Mathf.PI / 60f;

                if (wheelVelocity.magnitude < 1e-3f && wheelAngularVelocity * wheelCollider.radius < 1e-3f)
                    return Vector3.zero;

                var slipRatio = 1 - wheelVelocity.magnitude / (wheelAngularVelocity * wheelCollider.radius);
                var skidRatio = 1 - (wheelAngularVelocity * wheelCollider.radius) / wheelVelocity.magnitude;
                //slipRatio = skidRatio / (1 - skidRatio);
                slipRatio = Mathf.Clamp(slipRatio, -1, 1);
                float normalForce = wheelCollider.sprungMass;
                float frictionFactor = Mathf.Lerp(0.8f, 0.6f, Mathf.Abs(slipRatio));
                float tractionForce = normalForce * frictionFactor;
                

                var localWheelVelocity = wheelVelocity - wheelAngularVelocity * wheelHit.forwardDir;
                var tractionDir = -Mathf.Sign(Vector3.Dot(localWheelVelocity, wheelHit.forwardDir)) * wheelHit.forwardDir;
                Vector3 traction = tractionDir * tractionForce;

                if (this.name == "RearRightWheel")
                {
                    Debug.Log($"wheelAngularVelocity {wheelAngularVelocity}");
                    Debug.Log($"wheelVelocity {wheelVelocity}");
                    Debug.Log($"slipRatio {slipRatio}");
                    Debug.Log($"skidRatio {skidRatio}");
                    Debug.Log($"traction {traction}");
                    Debug.Log($"frictionFactor {frictionFactor}");
                    Debug.Log($"localWheelVelocity {localWheelVelocity}");
                }
                return traction;
            }
        }
        Vector3 localV = Vector3.zero;
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawRay(wheelHit.point, localV);
            Gizmos.color = Color.blue;
            Gizmos.DrawRay(wheelHit.point, lateralForce * 10);
            Gizmos.color = Color.red;
            Gizmos.DrawRay(wheelHit.point, dForce);

            //Debug.Log($"sprung mass {this.name} {wheelCollider.sprungMass} {wheelHit.force / 9.81f}");
        }
    }
}